...
|---|-----------|--------------|------------|
| I | Code Quality | Does this feature introduce any `any` types, ESLint suppressions, or undocumented public methods? | |
| II | Modular Architecture | Is the feature fully encapsulated in a NestJS module? Does it use the Repository pattern for DB access? Are DTOs defined for all I/O? | |
| III | Testing Discipline | Are unit tests planned for every Service method? Are integration tests planned for every Controller endpoint? Will coverage remain ≥ 70%? | |
| IV | API Contract | Are all endpoints prefixed `/api/v1/`? Are success/error envelopes used? Is Swagger documentation included in the plan? | |
